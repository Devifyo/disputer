        ...
        <div x-show="confirmingUserDeletion" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="confirmingUserDeletion" 
                     ...
                     class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full ring-1 ring-slate-900/5">
                    
                    <form method="post" id="delete-account-form" action="{{ route('profile.destroy') }}" class="p-6">
                        @csrf
                        @method('delete')
                        ...
                        <div class="mt-6">
                            ...
                            <div class="relative">
                                ...
                                <input id="password" name="password" type="password" 
                                    class="block w-full pl-10 pr-3 py-2.5 border border-slate-300 rounded-lg leading-5 bg-white placeholder-slate-400 focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20 sm:text-sm transition-all" 
                                    placeholder="{{ __('Enter your password to confirm') }}" 
                                    required />
                            </div>
                            @error('password') <p class="text-sm text-red-600 mt-2 font-medium">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="mt-6 sm:flex sm:flex-row-reverse gap-3">
                            ...
                            <button type="button" @click="confirmingUserDeletion = false" class="mt-3 w-full inline-flex justify-center rounded-lg border border-slate-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:text-sm transition-colors">
                                {{ __('Cancel') }}
                            </button>
                        </div>
                    </form>
