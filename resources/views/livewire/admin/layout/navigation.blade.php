<div>
    ...
    >
        ...
        <div class="h-20 flex items-center justify-between px-6 border-b border-white/5 bg-slate-950 shrink-0">
            ...
            {{-- Logo & Text Group --}}
            <div class="flex items-center gap-3 text-white min-w-0 overflow-hidden">
                <div class="w-8 h-8 rounded-lg bg-violet-600 flex items-center justify-center shadow-[0_0_15px_rgba(124,58,237,0.3)] shrink-0">
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2">
                        <span class="font-bold tracking-tight text-lg block leading-none truncate">{{ config('app.name') }}</span>
                        <span class="px-1.5 py-0.5 rounded text-[9px] font-bold bg-violet-900 text-violet-100 border border-violet-600/50 uppercase tracking-wider shrink-0">Admin</span>
                    </div>
                </div>
            </div>
            ...
            <button 
                wire:click="closeSidebar" 
                class="lg:hidden shrink-0 ml-2 p-1 rounded-md text-slate-400 hover:text-white hover:bg-white/10 transition-colors"
            >
        ...
        <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto sidebar-scroll">
            ...
            @php
                ...
                $navClass = fn($route) => request()->routeIs($route) 
                    ? 'bg-violet-600/10 text-violet-400 shadow-[inset_3px_0_0_0_#7c3aed]' 
            ...
            <a href="{{ route('admin.dashboard') }}" class="{{ $navClass('admin.dashboard') }} group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all">
                <i data-lucide="layout-dashboard" class="w-5 h-5 transition-colors {{ request()->routeIs('admin.dashboard') ? 'text-violet-400' : 'text-slate-500 group-hover:text-slate-300' }}"></i>
                Dashboard
            </a>

            <div class="px-2 mt-8 mb-3 text-[10px] uppercase tracking-wider font-bold text-slate-600 font-mono">Management</div>

            <a href="#" class="{{ $navClass('admin.users.*') }} group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all">
                <i data-lucide="users" class="w-5 h-5 transition-colors"></i>
                Users
            </a>
            ...
            <a href="#" class="{{ $navClass('admin.categories.*') }} group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all">
            ...
            <a href="#" class="{{ $navClass('admin.settings.*') }} group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all">
            ...
            <div class="flex items-center gap-1 p-2 rounded-xl bg-slate-900/50 border border-white/5 hover:border-white/10 transition-all">
                <div class="flex items-center gap-3 flex-1 min-w-0 p-1.5">
                    <div class="relative shrink-0">
                        <div class="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-sm text-white font-bold ring-2 ring-transparent">
                ...
                <button wire:click="logout" class="p-2.5 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors" title="Log Out">
