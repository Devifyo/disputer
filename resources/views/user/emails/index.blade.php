...
@section('content')
    <div class="h-[calc(100vh-4rem)] flex flex-col bg-slate-50/50">
        ...
        <div class="px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/50 backdrop-blur-sm border-b border-slate-200/60 sticky top-0 z-20">
            ...
            <div class="flex items-center gap-3">
                <div class="flex bg-slate-100 p-1 rounded-lg border border-slate-200">
                    <button class="px-3 py-1.5 rounded-md bg-white text-slate-900 shadow-sm text-xs font-bold ring-1 ring-slate-900/5 transition-all">All</button>
                    <button class="px-3 py-1.5 rounded-md hover:bg-white/50 text-slate-500 hover:text-slate-700 text-xs font-bold transition-all">Unread</button>
                    <button class="px-3 py-1.5 rounded-md hover:bg-white/50 text-slate-500 hover:text-slate-700 text-xs font-bold transition-all">Sent</button>
                </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 sm:p-6">
            <div class="max-w-6xl mx-auto bg-white rounded-2xl border border-slate-200 shadow-sm divide-y divide-slate-100 overflow-hidden">

                @foreach($threads as $email)
                <a href="{{ route('user.emails.show', $email->id) }}" class="group block relative hover:bg-blue-50/30 transition-all duration-200">
                    ...
                    <div class="flex items-start sm:items-center gap-5 p-5 pl-8"> <div class="shrink-0 relative">
                            @php
                                // Generate consistent colors based on the first letter
                ...
                <div class="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <h3 class="text-lg font-bold text-slate-900">Inbox Zero</h3>
                    <p class="text-slate-500 max-w-sm mt-1">Great job! You have no unread dispute updates.</p>
                </div>
                @endif
